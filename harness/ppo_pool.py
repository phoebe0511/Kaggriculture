...
def _init(cfg):
    ...
    from harness.ppo_rollout import build_net, load_opponent   # noqa: PLC0415

    torch.set_num_threads(1)
    _W["cfg"] = cfg
    _W["net"] = build_net(cfg["width"], cfg["blocks"])
    _W["opp"] = load_opponent(cfg["opponent"]) if cfg["opponent"] else None


def _run(job):
    """跑一輪。回傳 `(步數, [(我方現金, 對手現金)], 軌跡)`。"""
    import torch                                          # noqa: PLC0415
...
class RolloutPool:
    ...
    def collect(self, net, seed0):
        """回傳 `(步數, [(我方, 對手) 現金], 軌跡 list)`。"""
        state = {k: v.detach().cpu() for k, v in net.state_dict().items()}
        jobs = [(state, seed0 + i * self.envs) for i in range(self.workers)]
        steps, pairs, trajs = 0, [], []
