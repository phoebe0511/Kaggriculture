...
def generate(obs, config, base_action, rng, n_rank=4, multi_sizes=(2, 3, 5),
    ...
    markets = _market_variants(obs, config, mk_present, mk_qty, base_market)
    ...
    raw.append(("net-units", list(net_units), base_market))
    ...
    for i in range(n_units):
        for rank, alt in enumerate(_unit_ranked(op_l, qty_l, mask, i, n_rank)):
            ...
            units[i] = alt
            raw.append((f"u{i}r{rank}", units, base_market))
    ...
    for m in multi_sizes:
        ...
        for s in range(n_multi):
            ...
            raw.append((f"m{m}s{s}", units, base_market))
