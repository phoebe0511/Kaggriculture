...
DEFAULT_PARAMS = {
    ...
    # ------------------------------------------------------------------
    # 以下原本住在 `agents/gen1.py`（2026-08-21 合併進來）。
    #
    # gen1 從來就只是「gen0 + 一組調過的參數」——`act()` 解析完就直接呼叫
    # `gen0_act`。那一層間接沒有帶來任何行為差異，只讓每個讀 code 的人多問
    # 一次「這兩支差在哪」。下面每一項的證據註解都原樣保留。
    # ------------------------------------------------------------------
...
def _resolve_params(params=None):
    """`params` 疊在 `DEFAULT_PARAMS` 上，回傳這一局實際生效的參數。

    `_replace_defaults`：給 True 就**完全不套預設**，只用傳進來的那些。
    凍結量尺（`config/opponents/ref-*.json`）靠它把參數整組寫死，這樣改預設值
    不會回頭動到已經量過的基準 —— 但實測只有 `ref-v11` 真的展開了全部 51 項，
    其餘 ref 仍有 19 個 key 落回預設。詳見 2026-08-21 的 journal。

    2026-08-21 從 `agents/gen1.py` 搬過來（那支已刪）。`gen1` 舊版是先在自己
    那層解析、再把結果丟給 `gen0.act` 解析第二次；合併之後只解析一次，
    但結果逐項相同 —— 兩層都是「預設 update 覆寫」，冪等。
    """
    overrides = dict(params or {})
    # 🩸 `get` 不是 `pop` —— 標記要**留在回傳值裡**，由 `act()` 負責消掉。
    # 舊的 gen1 就是這個語意（先 `_resolve_params` 再交給 `gen0.act` 二次解析），
    # `tests/test_gen1_policy.py::test_frozen_full_params_replace_live_gen1_defaults`
    # 直接斷言了它。改成 pop 會讓那條測試紅，而且是真的行為變了。
    replace_defaults = bool(overrides.get("_replace_defaults", False))
    resolved = {} if replace_defaults else dict(DEFAULT_PARAMS)
...
def act(obs, config=None, params=None, return_plan=False, demand=None):
    ...
    **配對、走路、市場那三段完全共用** —— 那正是 v5 要保留的部分。
    ...
    p.pop("_replace_defaults", None)      # 標記不是參數，別讓它流進下游
