...
🔴 **2026-08-26：search 產得出更好的動作，但網路學不起來。** 這是目前的瓶頸。

    search 本身（08-26，20 局，12 支對手）
      對自己的對照組        20/20 局全贏   +25,871   p=1.9e-06   ← 🩸 恆等式
      對 8 位真實頂端玩家   現金比 0.615 -> 0.774                ← 這個才算數

🩸 **「N/N 局全贏」「配對差」「Wilcoxon p」是恆等式，不是證據**（08-27
journal §2）。`gain = q(贏家) − q(base)` 恆 ≥ 0，因為 base 自己就在候選裡
（`search/rollout_search.py:114-118` 強制）；而且相鄰項抵消之後
Σgain == 期末現金 − 對照組現金（10 局誤差全 0）。實測兩批共 1,770 次相鄰
決策點比較，`q` 一次都沒下降。**只有「現金比」那一欄有內容** —— 對手會跟著
動，那個沒有結構保證。

    蒸餾（round7 = round6 的配方 + 那 20 局）
      search 改掉 gen0 的盤面   0.7673 -> 0.7812   +1.4pp
      search 保留 gen0 的盤面   0.8766 -> 0.9109   +3.4pp
                                          差的差   −2.0pp

**在「標籤是 search 新發現的動作」上幾乎沒學到，反而是「標籤本來就是 gen0
動作」的進步比較多。** 根因是稀釋：600 / 1,016,666 = **0.059%**，99.94% 的
梯度在說「照 gen0 做」。那 3,803 個標籤就在 round7 的訓練集裡，**連背都沒背
起來**。判準工具是 `tools/search_label_probe.py`。

🔴 **2026-08-27 結案：標籤學不起來，因為它本身沒有可推廣的內容。**
`tools/search_curse_probe.py` 把 261 個「search 改掉動作」的決策點重評兩次
（A 換我方延續成 gen0；B 我方延續不變、只換對手），結果一致：

    原始 gain 的量存活   −1.7% / −1.6%
    原始 gain vs delta   Spearman r = −0.004 / +0.008   （零相關）
    兩批對同一個點的符號一致 52.0%，而互相獨立時就該是 52.4%

**argmax 挑的是「這一局走運的那條 30 天鏈」，不是比較好的動作**
（winner's curse）。雜訊 sd 是 4,572（同對手）/ 18,140（跨對手），而典型動作
差距是幾百元 —— 要分得出 500 元要 656 / 10,319 次 rollout，現在用 1 次。
**加權、加資料、換編碼都沒用。** 完整推導見 08-27 journal §6~§8。

🔴 **而且找到擺盪的實體機制了**（08-27 journal §11）：

    kaggriculture.py:871   rng = random.Random((seed * 1_000_003) ^ day)
    :836-839               _spawn_weeds 對**每一格空地**燒掉一次 rng.random()
    :891                   之後才 rng.choice(sorted(SHOPS)) 抽鎮上開哪間店

**種一格、收一格就改變空地數，就重抽一次店。** 而店決定 `_town_consume`
每 4 回合扣哪些產品的庫存，庫存決定市價（`:192-206`），市價是**兩家共用**的。

實測（`tools/search_ledger.py`，seed 7）：有搜 / 沒搜兩局的店完全不同，
STRAWBERRY 價 +47.7%、MILK +52.3%；對手是開迴路 replay、**賣的量一個沒變**，
收入卻多 57,645。**10 局全部都抽到不同的店。**

所以 `argmax(期末現金)` 有很大一部分在挑「哪個候選讓鎮上開出對我有利的店」
—— 純運氣，**網路不可能學會**（它看不到未來的 RNG）。

三條補救的路，兩條已經量掉：

| | 結果 |
|---|---|
| 減掉對手期末現金（control variate） | ❌ 上限 15%（ρ=0.53），直接相減反而更糟。§9 |
| 縮短 horizon + value head | ❌ value head 最好的情況 RMSE 16,140，是 4,572 的 3.5 倍。§10 |
| open-loop 延續 | ⬜ 沒量。能砍變異，但偏差方向跟現有缺陷疊加 |

🩸 **建議回到參數搜尋**（`tools/param_search.py` / `tools/noise_probe.py`，
08-25 建好的）：CMA-ES 調 gen0 參數。訊號在**整季**的尺度（配對差 sd
15,435~25,880），需要 54~197 個 seed —— 貴，但**是做得到的數量**；
search 那條路要 656 個延續才分得出一步，差兩個數量級。

08-25 留下的兩個未解問題：(1) **已結案** —— 增益不是靠壓低對手，兩種對手在
search 之後都變得更有錢（08-26 journal §6.1）；(2) 產出上不了場，一定要蒸餾
回網路 —— **就是這一步卡住了**。

    對 ladder-top-a（seed 0~9、a_slot 0、同一組 seed）的現金比
      round6 網路          51.5%   63,546
      gen1 / gen0 規則式    55.1%   67,944
      gen0 + 離線 search   76.9%   94,892     <- 🩸 那個 10/10、p=0.00195 是恆等式
      ladder-top-a 自己             123,323

🩸 **`ref-v3` / `ref-v4` 現在跑不動**（`KeyError: 'oversupply_factor'`，
08-26 journal §3）。固定 A 方用 `ref-v11`。

🩸 **用 `e2e-*` / `gen2_model` 當對手前要先設 `KAGGRI_WEIGHTS`** ——
`agents/gen2_model.py:111` 的 `WEIGHTS_PATH` 是模組層級常數、**import 當下
就決定**，沒設就退回不存在的 `agents/weights.npz`。自己驅動迴圈的工具要在
import `agents.gen2_model` **之前**設好。
